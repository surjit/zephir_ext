...
class Response extends Message implements ResponseInterface
{
     ...
     */
    public function __construct(int status = 200, <HeadersInterface> headers = null, <StreamInterface> body = null) -> void
    {
    ...
    public function withStatus(int code, string reasonPhrase = "")
    {
        ...
        let clone->status = code;
        if reasonPhrase === "" && isset static::messages[code] {
            let reasonPhrase = static::messages[code];
        }
        if reasonPhrase === "" {
            throw new InvalidArgumentException("ReasonPhrase must be supplied for this code");
        }
        let clone->reasonPhrase = reasonPhrase;
        return clone;
    }
    ...
    {
        var body, json, responseWithJson;
    
        let body =  this->getBody();
        body->rewind();
        let json =  json_encode(data, encodingOptions);
        body->write(json);
        // Ensure that the json encoding passed successfully
        if json === false {
            throw new \RuntimeException(json_last_error_msg(), json_last_error());
        }
        let responseWithJson =  this->withHeader("Content-Type", "application/json;charset=utf-8");
        if isset status {
    ...
    {
        var tmpArraye004c02dc951d634f7264a3859e95a42;
    
        let tmpArraye004c02dc951d634f7264a3859e95a42 = [204, 205, 304];
        return in_array(this->getStatusCode(), tmpArraye004c02dc951d634f7264a3859e95a42);
    }
    ...
    {
        var tmpArrayf3d7a27782facd241d2a3015c445e9c2;
    
        let tmpArrayf3d7a27782facd241d2a3015c445e9c2 = [301, 302, 303, 307];
        return in_array(this->getStatusCode(), tmpArrayf3d7a27782facd241d2a3015c445e9c2);
    }
    ...
    public function __toString() -> string
    {
        ...
        let output =  sprintf("HTTP/%s %s %s", this->getProtocolVersion(), this->getStatusCode(), this->getReasonPhrase());
        let output .= PHP_EOL;
        for name, values in this->getHeaders() {
            let output .= sprintf("%s: %s", name, this->getHeaderLine(name)) . PHP_EOL;
        }
        let output .= PHP_EOL;
