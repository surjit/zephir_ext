...
use Zesle\Psr\Http\Message\UploadedFileInterface;
use RuntimeException;
use Zesle\Psr\Http\Message\ServerRequestInterface;
use Zesle\Psr\Http\Message\UriInterface;
...
class Request extends Message implements ServerRequestInterface
{
     ...
     * @var string[]
     */
    protected validMethods = ["CONNECT" : 1, "DELETE" : 1, "GET" : 1, "HEAD" : 1, "OPTIONS" : 1, "PATCH" : 1, "POST" : 1, "PUT" : 1, "TRACE" : 1];
    /**
    ...
    {
        var method, uri, headers, cookies, tmpArray40cd750bba9870f18aada2478b24840a, serverParams, body, uploadedFiles, request, tmpArrayb91a6b199f63031173b76342f97e4427;
    
        let method = environment["REQUEST_METHOD"];
        let uri =  Uri::createFromEnvironment(environment);
        let headers =  Headers::createFromEnvironment(environment);
        let cookies =  Cookies::parseHeader(let tmpArray40cd750bba9870f18aada2478b24840a = [];
        headers->get("Cookie", tmpArray40cd750bba9870f18aada2478b24840a));
        let serverParams =  environment->all();
        let body =  new RequestBody();
        let uploadedFiles =  UploadedFile::createFromEnvironment(environment);
        let request =  new static(method, uri, headers, cookies, serverParams, body, uploadedFiles);
        let tmpArrayb91a6b199f63031173b76342f97e4427 = ["application/x-www-form-urlencoded", "multipart/form-data"];
        if method === "POST" && in_array(request->getMediaType(), tmpArrayb91a6b199f63031173b76342f97e4427) {
    ...
    public function __construct(string method, <UriInterface> uri, <HeadersInterface> headers, array cookies, array serverParams, <StreamInterface> body, array uploadedFiles = [])
    {
        ...
        let this->originalMethod =  this->filterMethod(method);
        let this->uri = uri;
        ...
        if isset serverParams["SERVER_PROTOCOL"] {
            let this->protocolVersion =  str_replace("HTTP/", "", serverParams["SERVER_PROTOCOL"]);
        }
        if !(this->headers->has("Host")) || this->uri->getHost() !== "" {
            this->headers->set("Host", this->uri->getHost());
        }
        this->registerMediaTypeParser("application/json", new Request__constructClosureOne());
        let backup =  libxml_disable_entity_loader(true);
        let result =  simplexml_load_string(input);
        this->registerMediaTypeParser("application/xml", new Request__constructClosureOne());
        let backup =  libxml_disable_entity_loader(true);
        let result =  simplexml_load_string(input);
        this->registerMediaTypeParser("text/xml", new Request__constructClosureOne());
        this->registerMediaTypeParser("application/x-www-form-urlencoded", new Request__constructClosureOne());
    }
    ...
    {
        var customMethod, body;
    
        if this->method === null {
            let this->method =  this->originalMethod;
            let customMethod =  this->getHeaderLine("X-Http-Method-Override");
            if customMethod {
                let this->method =  this->filterMethod(customMethod);
            } elseif this->originalMethod === "POST" {
                let body =  this->getParsedBody();
                if is_object(body) && property_exists(body, "_METHOD") {
                    let this->method =  this->filterMethod((string) body->_METHOD);
    ...
    public function withMethod(string method)
    {
        ...
        let method =  this->filterMethod(method);
        let clone =  clone this;
        let clone->originalMethod = method;
        let clone->method = method;
        return clone;
    }
    ...
    public function getRequestTarget()
    {
        ...
        let basePath =  this->uri->getBasePath();
        let path =  this->uri->getPath();
        let path =  basePath . "/" . ltrim(path, "/");
    ...
    public function withRequestTarget(requestTarget)
    {
        ...
        let clone =  clone this;
        let clone->requestTarget = requestTarget;
        return clone;
    }
    ...
    {
        var contentType, contentTypeParams, contentTypeParts, contentTypePartsLength, i, paramParts;
    
        let contentType =  this->getContentType();
        let contentTypeParams =  [];
        if contentType {
            let contentTypeParts =  preg_split("/\\s*[;,]\\s*/", contentType);
            let contentTypePartsLength =  count(contentTypeParts);
            let i = 1;
            for i in range(1, contentTypePartsLength) {
                let paramParts =  explode("=", contentTypeParts[i]);
                let contentTypeParams[strtolower(paramParts[0])] = paramParts[1];
            }
        }
    ...
    public function getContentCharset()
    {
        ...
        let mediaTypeParams =  this->getMediaTypeParams();
    ...
    public function getContentLength()
    {
        ...
        let result =  this->headers->get("Content-Length");
    ...
    public function withUploadedFiles(array uploadedFiles)
    {
        ...
        let clone =  clone this;
        let clone->uploadedFiles = uploadedFiles;
        return clone;
    }
    ...
    public function withAttributes(array attributes)
    {
        ...
        let clone =  clone this;
        let clone->attributes =  new Collection(attributes);
        return clone;
    }
    ...
    public function getParsedBody()
    {
        ...
        if isset this->bodyParsers[mediaType] === true {
            let parsed =  this->bodyParsers[mediaType](body);
     ...
     *     media type.
     */
    public function registerMediaTypeParser(string mediaType, callablee) -> void
    {
        if callablee instanceof Closure {
            let callablee =  callablee->bindTo(this);
        }
        let this->bodyParsers[(string) mediaType] = callablee;
    }
    ...
    {
        var postParams, getParams, result;
    
        let postParams =  this->getParsedBody();
        let getParams =  this->getQueryParams();
        let result = default;
        if is_array(postParams) && isset postParams[key] {
            let result = postParams[key];
        } elseif is_object(postParams) && property_exists(postParams, key) {
            let result =  postParams->{key};
        } elseif isset getParams[key] {
            let result = getParams[key];
        }
        return result;
    }
     ...
     * @return null
     */
    public function getParsedBodyParam(key, null default = null) -> null
    {
        var postParams, result;
     ...
     * @return null
     */
    public function getQueryParam(key, null default = null) -> null
    {
    ...
    public function getParams() -> array
    {
        ...
        let postParams =  this->getParsedBody();
        if postParams {
            let params =  array_merge(params, (array) postParams);
        }
        return params;
    }
