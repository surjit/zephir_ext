...
class UploadedFile implements UploadedFileInterface
{
     ...
     */
    protected static function parseUploadedFiles(array uploadedFiles) -> array
    {
        var parsed, field, uploadedFile, subArray, fileIdx, error;
    
        let parsed =  [];
        for field, uploadedFile in uploadedFiles {
            if !(isset uploadedFile["error"]) {
                if is_array(uploadedFile) {
                    let parsed[field] = static::parseUploadedFiles(uploadedFile);
                }
                continue;
            }
            let parsed[field] =  [];
            if !(is_array(uploadedFile["error"])) {
                let parsed[field] = new static(uploadedFile["tmp_name"],  isset uploadedFile["name"] ? uploadedFile["name"]  : null,  isset uploadedFile["type"] ? uploadedFile["type"]  : null,  isset uploadedFile["size"] ? uploadedFile["size"]  : null, uploadedFile["error"], true);
            } else {
                let subArray =  [];
                for fileIdx, error in uploadedFile["error"] {
                    // normalise subarray and re-parse to move the input's keyname up a level
                    let subArray[fileIdx]["name"] = uploadedFile["name"][fileIdx];
                    let subArray[fileIdx]["type"] = uploadedFile["type"][fileIdx];
                    let subArray[fileIdx]["tmp_name"] = uploadedFile["tmp_name"][fileIdx];
                    let subArray[fileIdx]["error"] = uploadedFile["error"][fileIdx];
                    let subArray[fileIdx]["size"] = uploadedFile["size"][fileIdx];
                    let parsed[field] = static::parseUploadedFiles(subArray);
                }
            }
        }
        return parsed;
     ...
     */
    public function __construct(string file, name = null, type = null, size = null, int error = UPLOAD_ERR_OK, bool sapi = false) -> void
    {
        let this->file = file;
        let this->name = name;
        let this->type = type;
    ...
    public function getStream() -> <StreamInterface>
    {
        ...
        if this->stream === null {
            let this->stream =  new Stream(fopen(this->file, "r"));
        }
        return this->stream;
    }
    ...
    public function moveTo(string targetPath) -> void
    {
        ...
        let targetIsStream =  strpos(targetPath, "://") > 0;
        if targetIsStream {
